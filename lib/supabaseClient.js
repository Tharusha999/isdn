...
export const fetchStaffDrivers = async () => {
  // Pull from driver_users since this is where the user manages their drivers
  const { data, error } = await supabase
    .from("driver_users")
  ...
  if (error) {
    console.error("Error fetching drivers from driver_users:", error);
    throw error;
  }
  ...
  return (data || []).map(d => ({
    id: d.id,
    name: d.full_name || d.username || "System Driver"
  }));
};
...
export const fetchPartners = async () => {
  try {
    ...
    // Fetch audits separately to avoid crashing if table is missing or join fails
    const { data: audits, error: auditsError } = await supabase
      .from("partner_audits")
      .select("*");

    const auditMap = {};
    if (audits && !auditsError) {
      audits.forEach(a => {
        if (!auditMap[a.partner_id]) auditMap[a.partner_id] = [];
        auditMap[a.partner_id].push(a);
      });
    }

    return partners.map(p => ({
      ...p,
      partner_audits: auditMap[p.id] || []
    }));
  } catch (err) {
...
export const createPartner = async (partnerData) => {
  // 1. Insert the organization profile
  ...
  // 2. Automatically create a login account for this partner
  // Generate a standard username from the email or name, ensure it has @partner.isdn
  const baseName = partnerData.name.toLowerCase().replace(/[^a-z0-9]/g, '');
  const username = `${baseName}@partner.isdn`;

  const loginData = {
    username: username,
    password: "RDC" + Math.floor(1000 + Math.random() * 9000), // Default password e.g. RDC4592
    full_name: partnerData.name,
    email: partnerData.email,
    phone: partnerData.phone,
    rdc_hub: partnerData.hub,
    is_active: true
  };
  ...
  if (loginError) {
    console.error("Warning: Created partner profile but failed to auto-generate login:", loginError);
    // We don't throw here to avoid failing the directory insertion, 
    // but the generated login data is returned in the object for the UI
  }

  // Attach the generated credentials to the response so the UI can show them
  return { ...partnerProfile[0], portal_username: username, portal_password: loginData.password };
};
...
export const fetchMissionsByDriver = async (driverId, driverName = null) => {
  let query = supabase
    .from("missions")
    .select(
      `
...
export const createMission = async (missionData) => {
  // Lazy Sync: Ensure the driver exists in the 'staff' table to satisfy foreign key constraint
  if (missionData.driver_id) {
    try {
      // Check if driver exists in staff registry
      const { data: staffExists, error: checkError } = await supabase
        .from("staff")
        .select("id")
        .eq("id", missionData.driver_id)
        .single();

      if (checkError && checkError.code !== 'PGRST116') { // PGRST116 is 'not found'
        console.warn("Staff lookup failed during lazy sync:", checkError);
      }

      if (!staffExists) {
        // Create an entry in the staff table to link the mission
        await supabase.from("staff").insert([{
          id: missionData.driver_id,
          name: missionData.driver_name || "Driver Node",
          role: "Driver",
          status: "Active",
          department: "Logistics",
          location: missionData.destination || "In Transit",
          join_date: new Date().toISOString().split('T')[0]
        }]);
      }
    } catch (err) {
      console.error("Lazy sync failed:", err);
      // We continue anyway, Supabase will throw a FK error if actually missing
    }
  }
  ...
  try {
    ...
    if (error) {
      ...
      const enhancedError = new Error(`Mission insert failed: ${error.message || 'Unknown Error'}`);
      enhancedError.details = error;
      throw enhancedError;
    }
    return data[0];
  } catch (err) {
    console.error("createMission critical error:", err);
    throw err;
  }
};
...
export const loginPartner = async (username, password) => {
  ...
  return {
    ...
    email: data.email,
    phone: data.phone,
    rdc_hub: data.rdc_hub,
    role: "partner",
    is_active: data.is_active
  };
};

// Fetch all orders for the partner dashboard.
// NOTE: The `rdc` parameter is kept for API compatibility but is NOT used as a filter,
// so all orders are visible to any partner regardless of which RDC they were assigned to.
export const fetchOrdersByPartnerRDC = async (rdc) => {
  try {
    ...
    const nameMap = {};
    if (customers) customers.forEach(c => { nameMap[c.id?.toLowerCase()] = c.full_name; });

    return orders.map(o => ({
      ...o,
      customer_name: nameMap[o.customer_id?.toLowerCase()] || "Retail Partner"
    }));
  } catch (err) {
    console.error("fetchOrdersByPartnerRDC error:", err);
    throw err;
  }
};

// Fetch active drivers (for partner driver assignment dropdown)
export const fetchActiveDrivers = async () => {
  const { data, error } = await supabase
    .from("driver_users")
    .select("id, full_name, Organization, Status")
    .eq("Status", "Active")
    .order("full_name", { ascending: true });
...
// Assign a driver to an order (partner action)
...
// Update order status (partner action)
export const updateOrderStatusByPartner = async (orderId, status) => {
  const { data, error } = await supabase
    .from("orders")
    .update({ status })
    .eq("id", orderId)
...
export const loginUser = async (username, password) => {
  ...
  // Pattern Check for Partner
  if (lowerUsername.includes("@partner.isdn")) {
    try {
      const partnerUser = await loginPartner(username, password);
      return { ...partnerUser, role: "partner" };
    } catch (e) {
      throw new Error("Invalid partner credentials");
    }
  }
...
export const fetchRDCHubs = async () => {
  const { data, error } = await supabase
    .from("rdc_hubs")
    .select("*")
    .order("name", { ascending: true });
  if (error) {
    console.error("fetchRDCHubs error", error);
    return [];
  }
  return data;
};

export const createRDCHub = async (name) => {
  const { data, error } = await supabase
    .from("rdc_hubs")
    .insert([{ name }])
    .select();
...
export const deleteRDCHub = async (id) => {
  const { error } = await supabase
    .from("rdc_hubs")
    .delete()
    .eq("id", id);
